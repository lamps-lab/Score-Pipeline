...
windows_pdftotext_path = '/home/cs_xwei001/.local/bin/pdftotext'
...
if __name__ == "__main__":
    ...
    args.data_file = r"/data/xwei/score_pileline_app/data_darpa1.csv"    #****************
    ...
    # Generate Training data
        ...
        file =pd.read_csv(r"/data/xwei/score_pileline_app/data_darpa1.csv")  #****************
        ...
        for i in range(0, len(doi)):
            ...
            y_dict[str(pdf_filename[i])] = y[i]
            issn_dict[str(pdf_filename[i])] = issn[i]
        ...
        for xml in xmls:
            try:
                ...
                extractor = TEIExtractor(args.grobid_out + '/' + xml, doi_dict, title_dict, database)
                # print('################################')
                # print(args.grobid_out + '/' + xml)
                # print('################################')
                extraction_stage = extractor.extract_paper_info()
                #issn = extraction_stage['ISSN']     # Xin
                
                issn = issn_dict[xml.split('.')[0]]     # Xin
                print('issn:', issn)                   # Xin
                ...
                # features['y'] = 1
                xmlindex = xml.split('.')[0]
                features['y'] = y_dict[xmlindex]
        ...
        dflist.to_csv('/data/xwei/score_pileline_app/out_csv/id.csv')
