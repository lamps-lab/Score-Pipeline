...
class PaperInfoCrawler:
    ...
    def addVenueFeatures(self, df, issn):
        ...
        #           'RANK', 'Rank Out Of']
        columns = ['Print ISSN', 'Citation Count', 'Scholarly Output', 'Percent Cited', 'CiteScore', 'SNIP', 'SJR',
                    'RANK', 'Rank Out Of', 'subject_code', 'subject']            # Xin modified
        all_venues = pd.read_csv(self.VENUE_METADATA_FILE)
        venue_info = all_venues[columns]
        
        #print('11111', venue_info['Print ISSN'][29])     # Xin
        #print(type(venue_info['Print ISSN'][29]))        # Xin
        #print('11111', venue_info['Print ISSN'][29].lstrip('0'))  # Xin
        #for index, row in venue_info.iterrows():           # Xin
        #    print(index)
        #    print(venue_info['Print ISSN'][index])         # Xin
        #    venue_info['Print ISSN'][index] = row['Print ISSN'].lstrip('0')     # Xin
        #    print(venue_info['Print ISSN'][index])         # Xin
            
            
        #for index, row in all_venues.iterrows():     # Xin
            #all_venues['Print ISSN'][index] = row['Print ISSN'].lstrip('0')     # Xin    
            #all_venues['E-ISSN'][index] = row['E-ISSN'].lstrip('0')
        ...
        if df_with_venue['CiteScore'].isnull().values.any():
            for index, row in df_with_venue.iterrows():        
                if len(all_venues[all_venues['E-ISSN'] == row['ISSN']]) != 0:
                    #print('ISSN in all venues:', row['ISSN'])      # Xin
                    a, b, c, d, e, f, g, h, i, j, k = all_venues[all_venues['E-ISSN'] == row['ISSN']][columns].iloc[0]          # Xin modified
                    ...
                    df_with_venue['subject_code'][index], df_with_venue['subject'][index] = j, k
                else:
        ...
        venue_name_dict = {'Citation Count': 'Venue_Citation_Count', 'Scholarly Output': 'Venue_Scholarly_Output',
                           ...
                           'SJR': 'Venue_SJR', 'subject': 'Venue_subject', 'subject_code': 'Venue_subject_code'}             # Xin modified
