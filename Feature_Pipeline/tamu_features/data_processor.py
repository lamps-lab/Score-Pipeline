...
import ast
import pdb
from Feature_Pipeline.tamu_features.sentiment_model import Sentiment
...
class DataProcessor:
    ...
    def process_auth_data(self, df, author_data):
        if len(author_data)==0:
           #return pd.DataFrame()
           df['avg_pub'], df['avg_hidx'], df['avg_auth_cites'], df['avg_high_inf_cites'] = -1, -1, -1, -1
           df['max_pub'], df['max_hidx'], df['max_auth_cites'], df['max_high_inf_cites'] = -1, -1, -1, -1
           df['first_pub'], df['first_hidx'], df['first_auth_cites'], df['first_high_inf_cites'] = -1, -1, -1, -1
           df['last_pub'], df['last_hidx'], df['last_auth_cites'], df['last_high_inf_cites'] = -1, -1, -1, -1
           return df
