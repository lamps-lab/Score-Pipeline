...
# TODO: Cleanup and documentation -> Rajal
...
class getelsevier(getcrossref):
    ...
    def check_dict(self, source):
        dict = pd.read_csv('/data/xwei/score_pileline_app/Feature_Pipeline/journal_dictionary.csv')
    ...
    def return_serialtitle(self):
        ...
        source = self.source
        query = issn
        url = 'https://api.elsevier.com/content/serial/title/issnn/' + issn + '?apiKey=4260067311461f46f378405531dbbab3'
        r = requests.get(url)
        status = r.status_code
